        ...
        if (sender instanceof Player) {
            ...
            if (args.length == 0) {
                ...
                if (args[0].equalsIgnoreCase("enable")) {
                    ...
                    if(player.hasPermission("meteor.medium") ||  player.hasPermission("meteor.op")) {
                        ...
                        if(isActivated) {
                            ...
                            Bukkit.broadcastMessage(ChatColor.GREEN + "The meteor spawned at " + ChatColor.RED + stand.getLocation().getX() + " " + ChatColor.AQUA + stand.getLocation().getY() + " " + ChatColor.YELLOW + stand.getLocation().getZ());
                            InventorySave.entityCheck(stand);
                        }else{
                            player.sendMessage(ChatColor.RED + "Enable the plugin in order to use this command!");
                        }
                    }else{
                        player.sendMessage(ChatColor.RED + "You don't have the permission to do this!");
                    }
